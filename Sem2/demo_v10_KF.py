...
def orient(cam, camera_matrix, distortion_coefficients, num_samples=15):
    tvecs_list = []
    rvecs_list = []
    
    print("Orienting system... collecting stable readings.")
    ...
    while True:
        ...
        if ids is not None:
            ...
            # Only collect if all required markers are visible
            if marker_ids.issubset(ids.flatten()):
                ...
                print(f"Collected {len(tvecs_list)}/{num_samples} valid readings", end='\r')
                
                if len(tvecs_list) >= num_samples:
                    # Convert to arrays for easier processing
                    tvecs_array = np.array(tvecs_list)
                    rvecs_array = np.array(rvecs_list)
                    
                    # Compute median instead of mean
                    median_tvecs = np.median(tvecs_array, axis=0)
                    median_rvecs = np.median(rvecs_array, axis=0)

                    
                    
                    print("\nSystem Oriented :) (median stabilized)")
                    print(median_tvecs.flatten())
                    return median_tvecs, median_rvecs, capture.color_timestamp_usec
        
        # Uncomment to visualize
        # cv2.imshow("Azure Kinect ArUco Localization", frame)
        
        if cv2.waitKey(1) & 0xFF == ord("q"):
            print("\nSystem Failed to Orient :(")
...
def save_log_to_json(data_log, filename="KFdata4.json"):
...
def main():
    ...
    while oriented:
        ...
        log_entry = {
                    'x': pose[1].item() if pose is not None else None,
                    "y": pose[0].item() if pose is not None else None,
                    'theta': pose[2].item() if pose is not None else None,
                    'filtered_x': filtered_pose[1].item() if filtered_pose is not None else None,
                    'filtered_y': filtered_pose[0].item() if filtered_pose is not None else None,
                    'filtered_theta': filtered_pose[2].item() if filtered_pose is not None else None,
                    'covariance': convert_for_json(KF.errorCovPost.copy())
        }
        ...
        if cv2.waitKey(1) & 0xFF == ord("q") or (t - first) / 1e6 >= 10:
            break
