...
def main(live_plot = 0, visualise = 0):
    ...
    if visualise:
        plt.ion()
        fig, ax = plt.subplots()
        point_robot, = ax.plot([], [], 'ro', label='Robot')
        point_person, = ax.plot([], [], 'bo', label='Person')
    ...
    while oriented:
        ...
        #         #print('this')
        #         #print(transformed_detection.flatten())
        #         #print(filtered_detection.flatten())
        ...
        #         last_detection = np.squeeze(filtered_detection.copy())
        ...
        #                                         [np.atan2(transformed_detection[1] - last_detection[1], 
        #                                          transformed_detection[0] - last_detection[0])]], np.float32)
        #             initial_base = initial_detection.copy()
        ...
        #     #test_variables = generateCollisionTestVariables(0,0,0,0, 0.01, 1000)
        #     test_variables = generateCollisionTestVariables(0.5, 0.5, 1, 0.5, 0.01, 200)
        #     #print(filtered_pose, forward_vel, filtered_detection, person_speed, test_variables)

        #     filtered_detection[2] = np.atan2(filtered_pose[1] - initial_base[1], filtered_pose[0] - initial_base[0])
        #     #print(filtered_pose[2])

        #     collision, collision_coords = testCollision(filtered_pose, forward_vel, filtered_detection, person_speed, test_variables)
        #     if visualise:
        #         heading_line_person.set_data([filtered_detection[0].item(), collision_coords[1][0].item()], [filtered_detection[1].item(), collision_coords[1][1].item()])
        #         heading_line_robot.set_data([filtered_pose[0].item(), collision_coords[0][0].item()], [filtered_pose[1].item(), collision_coords[0][1].item()])
        #         point_robot.set_data([filtered_pose[0]], [filtered_pose[1]])
        #         point_person.set_data([filtered_detection[0].item()], [filtered_detection[1].item()])
        #         if collision:
        #             heading_line_person.set_color('cyan')
        #             heading_line_robot.set_color('cyan')
        #         else:
        #             heading_line_person.set_color('blue')
        #             heading_line_robot.set_color('blue')

        # elif visualise:
        #     point_robot.set_data([filtered_pose[0]], [filtered_pose[1]])
        #     point_person.set_data([], [])
        #     heading_line_person.set_data([], [])
        #     heading_line_robot.set_data([], [])
