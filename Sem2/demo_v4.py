...
def main():
    ...
    while oriented:
        ...
        if pose is not None:
            ...
            #print(pose)
            coordinate, frame = detect(k4a, capture, model, frame)
            if coordinate:
                point2.set_data([coordinate[0]], [coordinate[1]])

                transformed_coordinate = transform(pose, coordinate)
                point3.set_data([transformed_coordinate[0]], [transformed_coordinate[1]])

                

            #point2.set_data([transformedObject[0]], [transformedObject[2]])
